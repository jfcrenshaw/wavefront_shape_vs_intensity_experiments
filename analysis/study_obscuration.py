...
def sweep(eps_values, n_mc, seed, n_jobs):
    ...
    for i, eps in enumerate(eps_values):
        ...
        res = sim.monte_carlo(C.DENSE_TERMS, kwargs, n_mc=n_mc, seed=seed,
                              n_jobs=n_jobs, flux_norm="per_pixel", z_ref=z_ref)
        residuals[i] = res
...
def plot_family(eps_values, sig, residuals, styles, title, fname):
    ...
    for term, label, color, filled in styles:
        ...
        kwargs = dict(marker="o", color=color, linestyle="-",
                      markerfacecolor=color if filled else "white",
                      label=label)
        lo, hi = plotutils.relative_error_interval(residuals, j)
        ax.errorbar(eps_values, curve,
...
def main():
    ...
    if args.plot_only:
        ...
        defocus_z4 = np.array([
            sim.fixed_diameter_defocus(eps * C.R_OUTER)
            for eps in eps_values
        ])
        np.savez(datafile, eps_values=eps_values, sig=sig, residuals=residuals,
